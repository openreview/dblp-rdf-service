...
from dblp_service.pub_formats.rdf_tuples.trees import get_nth_descendent, iter_subject_triples
...
def test_iter_subject_triples_simple():
    ...
    for i, (n1, n2, n3) in enumerate(iter_subject_triples(root)):
        actual = f"{n1.node_name} {n2.node_name} {n3.node_name}"
        assert actual == expected[i]


def test_iter_subject_triples_nested():
    path_list = [
        "s/r/so/r1/o",
        "s/r/so/r2/o",
    ]
...
def test_iter_subject_triples_depths():
    examples = [
        ["sub1/rel1/obj1"],
        ["r1/sub1/rel1/obj1"],
    ...
    for exnum, example in enumerate(examples):
        ...
        for n1, n2, n3 in iter_subject_triples(desc):
